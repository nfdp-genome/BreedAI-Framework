...
run_phase1() {
  ...
  echo "  ${REPO_ROOT}/dataset/input/Geno.csv"
  echo "  ${REPO_ROOT}/dataset/input/Pheno.csv"
  echo
  echo "Copy benchmark files from dataset/public_datasets/.../ when reproducing a public run"
  echo "(see dataset/public_datasets/cattle/README.md for the cattle benchmark)."
  echo

  if [[ ! -f "${REPO_ROOT}/dataset/input/Geno.csv" ]] || [[ ! -f "${REPO_ROOT}/dataset/input/Pheno.csv" ]]; then
    echo "ERROR: Geno.csv and/or Pheno.csv missing under ${REPO_ROOT}/dataset/input/"
...
run_phase2() {
  ...
  read -r -p "Path to new animals genotype CSV file (Enter = use dataset/input/Geno.csv): " new_geno
  if [[ -z "${new_geno}" ]]; then
    new_geno="${REPO_ROOT}/dataset/input/Geno.csv"
...
test_setup() {
  ...
  echo "  - Phase 1 default / default_plus_rnd (dataset/input)"
