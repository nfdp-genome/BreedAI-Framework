...
INPUT_DIR="$PROJECT_DIR/dataset/input"
...
# Data files - check dataset/input/ first, then data/
X_FILE=""
Y_FILE=""
...
if [[ -f "$INPUT_DIR/Geno.csv" ]] && [[ -f "$INPUT_DIR/Pheno.csv" ]]; then
    ...
    echo "✓ Data files found in dataset/input/"
    ...
    echo "  - dataset/input/ (preferred location)"
    echo "  - data/ (fallback)"
    exit 1
fi
