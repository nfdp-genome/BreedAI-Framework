...
# cannot use $PROJECT_DIR (unlike the pred_prep script generated below, which is templated
# at runtime and does). Submitting from anywhere but scripts/ therefore writes the SLURM
# logs outside the repo. The run itself is unaffected, so warn rather than exit.
